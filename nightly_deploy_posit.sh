...
git add pediatric_trials_cache.rds trials_cache.rds
git diff --cached --quiet && log "No cache changes, skipping commit." || \
    git commit -m "chore: nightly cache update $(date '+%Y-%m-%d')"
git push
