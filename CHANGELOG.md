...
### Sponsor normalisation — deep queue curation + pipeline fixes

**Queue reduced from 553 → 102 rows (81%); accepted rate 93.4% (15,532 / 16,594).**

#### Config changes

- **`final_sponsor_canonical_map.csv`** (+73 entries): collapsed multi-canonical hospital clusters to single authoritative forms — Erasme variants → `Hôpital Erasme`, Rijnstate → `Rijnstate`, Catharina → `Catharina Ziekenhuis`, Erlangen → `Universitätsklinikum Erlangen`, Essen hospital → `Universitätsklinikum Essen`, Münster → `Universitätsklinikum Münster`, University of Essen → `University of Duisburg-Essen`, Kliniken Essen-Mitte; plus 44 subsidiary/variant→parent collapses (AGI Therapeutics Research, BioNTech Cell & Gene Therapies, Fresenius Kabi Deutschland → Fresenius Kabi, McNeil → Johnson & Johnson, Mereo BioPharma 1–4 → Group, South Manchester → Manchester University NHS Foundation Trust, Stealth Peptides → Stealth BioTherapeutics, etc.).
- **`final_sponsor_family_map.csv`**: updated 15 stale `sponsor_clean_to` entries to match the new canonical forms, resolving false entity-key ambiguities.
- **`sponsor_llm_reviewed.csv`** (+129 entries): explicit canonical assignments for `final_family_map` review queue entries (Cologne, Saint-Luc, Erlangen, Erasme, Essen, Münster, Rijnstate, Catharina, CHU de Liège, Herlev og Gentofte, Bispebjerg, Parker Institute, IIB Sant Pau, etc.); department→parent mappings for Rigshospitalet, Charité, MUW/AKH Wien → Medical University of Vienna, Erasmus MC, VUmc → Amsterdam UMC, and 30+ other institutions; pharma alias confirmations (CHIESI, GENMAB, Galapagos, etc.); 40 additional entries for small pharma/typos/institutions (GEMCAD, Pifzer→Pfizer, AGI Therapeutics typo, Hospital Universitario de Rennes, etc.).
- **`sponsor_negative_aliases.csv`** (+340 entries): ~130 person names (individual investigators listed as sponsor), ~200 generic/irresolvable department labels without parent context, combined multi-entity sponsor strings.
- **`sponsor_ambiguous_aliases.csv`**: cleared; 3 entries (Air Liquide Santé International, Bracco Imaging, Recordati Rare Diseases) are regenerated by the EPAR MAH tier on each rebuild — structurally unavoidable without an EPAR-level override.
- **`manual_sponsor_aliases.csv`**: raised `confidence_prior` 0.9→1.0 for 7 unambiguous short-form pharma aliases (chiesi, galapagos, genmab, teva, abbott, astellas, biomarin).

#### Pipeline fixes

- **`normalise_sponsors.R`**: swapped `check_alias` before `check_family_entity` in `normalise_one()`. Exact alias matches now always take priority; the family entity check is a fallback only. This fixed ~36 entries showing as `final_family_map` ambiguous despite having exact alias matches.
- **`build_sponsor_labels.R`**: (1) Added guard for missing `decision`/`canonical_sponsor`/`comment` columns when reading an existing queue file — fixes crash on second run. Queue output now always writes all 11 columns. (2) Added filter to exclude `"generic department label without parent institution"` entries from the review queue — these 155 irresolvable entries were cluttering the queue with no actionable information.

### Normalisation pipeline — raw-value fallback for unmatched trials

- **Sponsor pipeline**: `build_sponsor_labels.R` now includes a raw-value fallback for trials whose sponsor resolves to `unknown` (no alias/family/fuzzy match). Those trials are added to `trial_sponsor_labels.csv` with `sponsor_clean = raw_sponsor` and `match_status = "unknown"`, so the app always shows something (e.g. 2025-522469-31-00) rather than a blank.
- **Substance pipeline**: `build_substance_labels.R` applies the same logic — trials with no accepted substance label receive a fallback `substance_label` built from their raw substance strings (filtered through the existing `is_exploratory_substance()` guard). Both scripts report the fallback row count in their console output.
