...
helper_scripts/substance_norm_pipeline/
  ...
  export_raw_substances.R     extracts raw substances from trials_cache.rds for testing
...
- `config/substance_norm_pipeline/ambiguous_substance_aliases.csv` — review these; they map to multiple substances
...
tibble(
  ...
  match_source,             # canonical | alias_index | manual | fuzzy:* | placebo_rule | ...
...
After a cache rebuild, `config/substance_norm_pipeline/substance_review_queue.csv` contains all rows with `match_status` of `review` or `unknown`, sorted by frequency.
